...
class Daily extends Command {
    ...
    async execute(interaction, client, lng) {
        ...
        data.currency = (data.currency || 0) + dailyReward;
        data.daily = now;
        await data.save();

        const embed = new EmbedBuilder()
            .setColor(Colors.NotQuiteBlack)
            .setTitle(`🎉 Vous avez récupéré **${dailyReward} <:currency:1319403790281080843>** pour votre récompense quotidienne !\n*Solde actuel: \`${data.currency}\`*`)
